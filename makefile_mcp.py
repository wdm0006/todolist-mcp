...
def parse_cli_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="Makefile",
        help="Path to the Makefile (default: Makefile in current directory)"
    )
    parser.add_argument(
        "--include",
        type=str,
        help="Comma-separated list of targets to include (default: all targets)"
    )
    parser.add_argument(
        "--exclude",
        type=str,
...
def list_available_targets() -> Dict[str, Any]:
    ...
    return {
        ...
        "working_directory": str(WORKING_DIR),
        "total_targets_in_makefile": len(parser.get_targets()),
        "available_targets": len(filtered_targets),
        "targets": [
            {
                "name": name,
                "description": desc,
                "tool_name": f"make_{name.replace('-', '_').replace('.', '_')}"
            }
...
def get_makefile_info() -> Dict[str, Any]:
    ...
    """
    all_targets = parser.get_targets()
